...
import { Trophy, Compass, Flame, Shield, ArrowUpRight, Award, Zap, Quote } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-10">
          ...
          <div className="flex flex-col sm:items-end font-mono text-xs text-rb-silver max-w-sm">
            ...
            <p className="text-rb-muted text-left sm:text-right">
              World champions and X-Games gold medalists performing unprecedented stunts across Egyptian airspace and marine reefs.
            </p>
          </div>
        </div>
        ...
        <div className="glass-panel-accent p-6 sm:p-10 rounded-3xl border border-white/20 shadow-2xl grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative overflow-hidden">
          {/* Left Column: Bio, Quotes, and Performance Telemetry */}
          <div className="lg:col-span-7 flex flex-col justify-between">
            <div>
              <div className="flex flex-wrap items-center gap-2.5 mb-4">
                <span className="px-3 py-1 rounded-full bg-white/10 text-white font-mono text-[10px] tracking-widest uppercase flex items-center gap-1.5">
                  ...
                  <span className="font-bold">{current.country}</span>
                </span>
                <span className="px-3 py-1 rounded-full bg-rb-red/20 text-rb-red font-mono text-[10px] font-bold tracking-widest uppercase border border-rb-red/30">
                  {current.titles}
                </span>
                <span className="text-[10px] font-mono text-rb-muted uppercase tracking-wider">
                  ARENA: {current.arena.split(' ')[0]}
                </span>
              </div>

              <h3 className="text-3xl sm:text-5xl font-display font-black text-white uppercase tracking-tight mb-4 leading-none">
                {current.name}
              </h3>
              ...
              {/* Quote block */}
              <blockquote className="relative p-4 rounded-xl bg-white/5 border-l-2 border-rb-yellow font-mono text-xs text-white/90 italic mb-6 pl-5">
                <Quote className="w-4 h-4 text-rb-yellow/40 absolute -top-2 left-2" />
                "{current.quote}"
              </blockquote>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-4 border-t border-white/10">
              {current.stats.map((s) => (
                <div key={s.label} className="glass-panel p-3.5 rounded-xl border border-white/5 bg-rb-dark/40">
